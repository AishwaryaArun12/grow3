...
const NewMsg = ({conversation,sender,set}) => {
    ...
    useEffect(()=>{
        console.log('ggggggggggg',conversation.members,conversation.members.some(member => member._id === arrivalMessage?.sender,'lllllllllllllllllllllllllllll'))
        if(arrivalMessage && conversation.members.some(member => member._id === arrivalMessage.sender)){
            console.log('cddddddddddddddddddddddddddddddddddd')
            set();
        }
  ...
  return (
    <div className="flex flex-row items-center h-16 rounded-xl fixed bottom-5 left-1/4  bg-white w-3/5 px-4">
    ...
    <div className="flex-grow ml-4">
      <div className="relative w-full">
        <input onChange={(e)=>{setValue(e.target.value)}} value={value} type="text" className="flex w-full border rounded-xl focus:outline-none focus:border-indigo-300 pl-4 h-10" />
