...
import { FaPaperPlane, FaTimes, FaHeart } from 'react-icons/fa';
...
const Comment = ({comment}) => {
    ...
    const formatDate = (postDate) => {
      ...
      const toggleColour = async(event) => {
        const icon = event.currentTarget;
          try {
            await axios.patch('/post/comment_like',{id : comment._id,userId : user._id})
            getAllPosts();
      ...
      const remove = async()=>{
        try {
          await axios.delete(`/post/delete_comment/${comment._id}`);
          getAllPosts();
        } catch (error) {
          console.log(error);
        }
  ...
  return (
    <article className="p-6 text-base bg-white rounded-lg dark:bg-gray-900">
      <footer className="flex justify-between items-center mb-2">
       ...
       {localStorage.getItem('id') != 'ADMIN' && <button
          ...
          onClick={()=>{setDropdown(!dropdown); setEditInput(false)}}
          className="z-10 inline-flex items-center p-2 text-sm font-medium text-center text-gray-500 dark:text-gray-400 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-50 dark:bg-gray-900 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
          type="button"
        >
        ...
        >
          <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconHorizontalButton">
           {user?._id == comment.userId._id ? <> <li>
            ...
            <li>
              <div>
              <Modal confirmation='Are you sure you want to remove this comment ?'  body={Confirm} updatePost={remove}
      ...
      <div className="flex items-center mt-4 space-x-4">
        ...
        <button className="flex justify-center items-center gap-2 px-2 hover:bg-gray-50 rounded-full p-1">
            <FaHeart onClick={localStorage.getItem('id') != 'ADMIN' ? toggleColour : ''}   className={`${comment.likes?.includes(user?._id)? 'text-red-700': 'text-gray-500'} w-5 h-5 fill-current`}/>
      ...
      <span>
          ...
          {emoji && <div className='absolute z-50'><Picker   data={data} onEmojiSelect={(emoji)=>{setReplyComment(prev=>prev+emoji.native);}} /></div>}
        </span>
    ...
    <label htmlFor="floating_outlined" className="absolute text-lg text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Edit Comment</label>
    </div>
